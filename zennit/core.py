...
@contextmanager
def mod_params(module, modifier, in_modifier, param_keys=None, require_params=True):
        ...
        for key in param_keys:
            if key not in missing:
                ...
                if param is not None:
                    ...
                    if key == "bias":
                        setattr(module, key, torch.nn.Parameter(modifier(param.data) * in_modifier(torch.ones_like(param.data))))
                    else:
                        setattr(module, key, torch.nn.Parameter(modifier(param.data)))
        yield module
    finally:
...
class Hook:
    ...
    def __init__(self):
        self.stored_tensors = {}
        self.weight_rule = None

    def set_weight_rule(self, weight_rule):
...
class BasicHook(Hook):
    ...
    def backward(self, module, grad_input, grad_output):
        ...
        # CALCULATE WEIGHT RELEVANCE
        if self.weight_rule is not None:
            params = dict(zip([name for name, _ in module.named_parameters()], [[] for _ in module.parameters()]))
            params_grad = dict(zip([name for name, _ in module.named_parameters()], [[] for _ in module.parameters()]))

            for in_mod, param_mod, out_mod in zip(self.weight_rule.input_modifiers,
                                                  self.weight_rule.param_modifiers,
                                                  self.weight_rule.output_modifiers):
                input = in_mod(original_input).requires_grad_()
                param_kwargs["in_modifier"] = in_mod
                ...
                with mod_params(module, param_mod, **param_kwargs) as modified, torch.autograd.enable_grad():
                    ...
                    for name, weight in modified.named_parameters():
                        params[name].append(torch.clone(weight))
                        relnorm = False ## imitates lrp_quant_tests_costf_relnorm branch TODO
                        if relnorm:
                            grad_out = grad_output[0].clone()
                            if len(grad_out.shape) > 3:
                                grad_out = grad_out.div(grad_out.abs().sum((1, 2, 3))[:, None, None, None]).mul(
                                    grad_out.sum((1, 2, 3))[:, None, None, None].abs())
                        else:
                            grad_out = grad_output[0]
                        param_grad = torch.autograd.grad(output,
                                                         weight,
                                                         grad_outputs=self.weight_rule.gradient_mapper(grad_out,
                                                                                                       [output]),
                                                         retain_graph=True)
                        params_grad[name].append(param_grad[0])

            for name, weight in module.named_parameters():
                weight.relevance = self.weight_rule.reducer(params[name], params_grad[name])
                #print(weight.relevance.sum(), weight.relevance[weight.relevance==0].numel(), weight.relevance.numel())

        # CALCULATE INPUT RELEVANCE
        ...
        module.input_relevance = relevance
        module.output_relevance = grad_output[0]

        """ CALCULATE WEIGHT RELEVANCE VIA CHANNEL SUM
        for name, weight in module.named_parameters():
            if len(weight.shape) > 2:
                channel_relevance = relevance.abs().sum((0, 2, 3)).unsqueeze(0).unsqueeze(2).unsqueeze(3).expand_as(weight)
                weight.relevance = weight.abs() * channel_relevance
            if len(weight.shape) == 2:
                channel_relevance = relevance.abs().sum(0).unsqueeze(0).expand_as(weight)
                #weight.relevance = weight.abs() * channel_relevance
        """
        # print(module, relevance.sum(), grad_output[0].sum())
        # relevance = relevance/relevance.sum() * grad_output[0].sum()
...
class Composite:
    ...
    '''
    def __init__(self, module_map=None, canonizers=None, weight_rule=None):
    ...
    def register(self, module):
        ...
        for name, child in module.named_modules():
            ...
            if template is not None:
                hook = template.copy()
                hook.set_weight_rule(self.weight_rule)
